  ...
  */
  ...
  {           
   ...
   { 
    ...
    if ( (fldNo > 0) && (fldNo <= fldCnt))      
     {
        val = Convert.getStrValue(fldOffset[fldNo -1], data, Size.STRINGSIZE); //strlen+2
        return val;
     }
    else 
     throw new FieldNumberOutOfBoundException (null, "TUPLE:TUPLE_FLDNO_OUT_OF_BOUND");
   ...
   */
   ...
   /** setTupleMetaData will set metadata of this Tuple object
    *  @param	tuple_length	length of tuple
    *  @param	fldCnt			number of fields 
    *  @param	fldOffs			offset of fields in data byte array
    */
 ...
 throws IOException, InvalidTypeException, InvalidTupleSizeException		
{
    ...
    throws IOException 
 {
  ...
  for (i=0; i< fldCnt-1; i++)
   ...
   case AttrType.attrString:
     sval = Convert.getStrValue(fldOffset[i], data,Size.STRINGSIZE);
...
public void setColHdr (short numFlds,  AttrType types, short strSizes)
throws IOException, InvalidTypeException, InvalidTupleSizeException		
{
 if((numFlds +2)*2 > max_size)
   throw new InvalidTupleSizeException (null, "TUPLE: TUPLE_TOOBIG_ERROR");
 ...
 int i;
  ...
  case AttrType.attrString:
    incr = (short) (strSizes +2);  //strlen in bytes = strlen +2
    strCount++;
    break;       
 ...
 Convert.setShortValue(fldOffset[1], pos, data);
 pos +=2;
...
switch(types.attrType) {
 ...
 fldOffset[numFlds] = (short) (fldOffset[0] + incr);
 Convert.setShortValue(fldOffset[numFlds], pos, data);
