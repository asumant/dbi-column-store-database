...
public class Heapfile implements Filetype,  GlobalConst {
  ...
  public HashMap<Integer, Integer> pagerank;
  ...
  */
  private HFPage _newDatapage(DataPageInfo dpinfop)
	...
	throw new HFException(null, "can't new pae");
      ...
      pagerank.put(pageId.pid, currentRank);
  ...
  */
  ...
  public  Heapfile(String name) 
	   ...
	   
    {
      ...
      _file_deleted = true;
      _fileName = null;
      pagerank = new HashMap<Integer, Integer>();
      if(name == null) 
	{
      ...
      if(_firstDirPageId==null)
	{
	  ...
	  SystemDefs.newFile = true;
	  /* Initialize currentRank to one. Current Rank denotes 
	   * how many data pages are allocated in this heap file*/
...
//      return rid;
	  ...
	  return checkPostoRid(pos);
  }
  
  public RID checkPostoRid(int pos)
  {
	  try
	  {
	  int position = 0;
	  ...
	  while (currentPage != null) {
		  flag = false;
	  for (currentDataPageRid = currentPage.firstRecord();
      position<pos;currentDataPageRid = 
	  currentPage.nextRecord(currentDataPageRid))
	  {
		 if(currentDataPageRid == null)
		 {
